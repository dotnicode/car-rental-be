...
export class AWSS3StorageService implements IStorageService {
   ...
   *
   * @param file - The file to upload (allowed types: PDF (.pdf), Word (.doc/.docx), JPEG (.jpg, .jpeg), PNG (.png))
  ...
  async uploadFile(file: Express.Multer.File): Promise<IFileUploadResult> {
    ...
    const allowedMimeTypes = ['application/pdf', 'application/msword', 'image/jpeg', 'image/png'];
